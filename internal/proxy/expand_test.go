...
func TestExpandFullAndPath(t *testing.T) {
	...
	if err != nil { t.Fatalf("full: %v", err) }
	if !strings.Contains(full, `"id":"a"`) { t.Fatalf("missing data: %s", full) }
	one, err := expand(ctx, s, "credA", ExpandArgs{Handle: h, Path: "data[1].id"})
	if err != nil { t.Fatalf("path: %v", err) }
	if strings.TrimSpace(one) != `"b"` { t.Fatalf("path = %s", one) }
}

func TestExpandRejectsForeignCredential(t *testing.T) {
	s := store.NewMemory(); ctx := context.Background()
