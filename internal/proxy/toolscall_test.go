...
func TestOptimizeSmallPassesThrough(t *testing.T) {
	...
	if out.Text != `{"ok":true}` || out.Compacted { t.Fatalf("small should pass through: %+v", out) }
}
...
func TestOptimizeHeavyCompactsAndCaches(t *testing.T) {
	...
	if err != nil { t.Fatalf("err: %v", err) }
	if !out.Compacted { t.Fatalf("heavy should compact") }
	if out.Handle == "" { t.Fatalf("expected handle") }
	if !strings.Contains(out.Text, "expand_result") { t.Fatalf("expected trailer") }
	if _, err := mem.Get(context.Background(), out.Handle); err != nil { t.Fatalf("raw should be cached: %v", err) }
}
...
func TestOptimizeOversizeFailsOpen(t *testing.T) {
	...
	if err != nil { t.Fatalf("err: %v", err) }
	if out.Compacted || out.Handle != "" { t.Fatalf("oversize must fail open, no handle") }
	if out.Text != string(raw) { t.Fatalf("oversize must return full raw") }
}
...
func TestShadowModeReturnsFullButCaches(t *testing.T) {
	...
	out, err := o.process(context.Background(), "credA", "list-tool", bigArray())
	if err != nil { t.Fatalf("err: %v", err) }
	if out.Compacted { t.Fatalf("shadow must not compact client-facing result") }
	if !strings.Contains(out.Text, `"status":"active"`) { t.Fatalf("shadow must return full result") }
}
